...
public class Player : Character, IDamageable
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        // {
        //     Vector2 position = new(0, flyPower);
    ...
    //    {
    //        Collider2D[] overlappedColliders = Physics2D.OverlapCircleAll(meleeAttackOrigin.position, meleeAttackRadius, shopLayer);
    //        Debug.Log(overlappedColliders);
    ...
    private void HandleAnimations()
    {
        ...
        if (attemptMeleeAttack && timeStartAttack <= 0)
        {
            if (!isMeleAttacking)
            {
        ...
        if(attemptJump && CheckGrounded() || Rb.velocity.y>1f)
        {
            if (!isMeleAttacking)
            {
